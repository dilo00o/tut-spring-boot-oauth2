...
In this section, we modify the <<_social_login_manual,app>> we have already built by adding a link so the user can choose to authenticate with Github, in addition to the original link to Facebook.

== Adding the Github Link

In the client, the change is trivial. We need only add another link, as follows:
...
field in the same place in their user info, so there is no change
...
already have a custom authentication filter for Facebook in
our `ssoFilter()` method, so all we need to do is replace that with a
composite that can handle more than one authentication path, as follows:
...
The code from our old `ssoFilter()` has been duplicated, once
for Facebook and once for Github, and the two filters have been merged into a
composite.

Note that the `facebook()` and `facebookResource()` methods have been
supplemented with similar methods named `github()` and `githubResource()`, as follows:
...
We also need to adjust the corresponding configuration, as follows:
...
https://github.com/settings/developers[Github] and with the address
`localhost:8080` (same as for Facebook).

The application is now ready to run and gives the user the choice between
authenticating with Facebook or Github.

== How to Add a Local User Database

Many applications need to hold data about their users locally, even if
authentication is delegated to an external provider. We do not show the
code here, but it is easy to do in two steps:

. Choose a backend for your database, and set up some repositories
(for example, by using Spring Data) for a custom `User` object that suits your
...
updated. Otherwise, it can be created.
